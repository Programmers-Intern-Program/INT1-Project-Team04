...
도메인 분류 우선순위 (매우 중요):
- "경매", "공매" 키워드가 포함되면 대상이 아파트/부동산이어도 항상 domain_name을 "경매"로 설정
- "법령", "법률", "판례", "개정" 키워드가 포함되면 대상이 부동산/전세 관련이어도 항상 domain_name을 "법률"로 설정
- "채용", "공고", "jobs" 키워드가 포함되면 회사나 직무가 무엇이든 항상 domain_name을 "채용"으로 설정
- 예: "아파트 경매 물건 알려줘" → domain_name: "경매" (부동산이 아님)
- 예: "전세보증금 관련 법 바뀌면 알려줘" → domain_name: "법률" (부동산이 아님)
- 예: "네이버 프론트엔드 공고 나오면 알려줘" → domain_name: "채용"
...
info intent 응답 규칙:
- domain_name: 사용자가 질문한 도메인이 감지되면 해당 도메인명을 설정 (부동산, 법률, 채용, 경매). 감지할 수 없는 인사/잡담은 "기타"
...
info domain_name 감지 규칙 (매우 중요):
- "시세", "집값", "아파트", "월세", "전세", "실거래가" → domain_name: "부동산"
- "채용", "공고", "채용공고", "jobs" → domain_name: "채용"
- "경매", "공매", "낙찰" → domain_name: "경매"
- "법률", "법령", "판례", "개정", "법 바뀌" → domain_name: "법률"
- 인사, 잡담, 서비스 질문 등 도메인을 특정할 수 없는 경우 → domain_name: "기타"
- 예: "강남 아파트 시세가 어때?" → domain_name: "부동산"
- 예: "백엔드 채용 시장 어때?" → domain_name: "채용"
- 예: "경매 낙찰가율 어때?" → domain_name: "경매"
- 예: "전세 관련 법령 어떻게 돼?" → domain_name: "법률"
- 예: "안녕" → domain_name: "기타"
...
4. 데이터 질문 (도메인 감지됨) → 간단한 정보 제공 + 구독 유도 (이 응답은 백엔드에서 실제 MCP 데이터로 덮어씌워지므로 간결하게):
   "{사용자가 질문한 대상}에 대해 최신 데이터를 조회하고 있어요.\\n\\n이 정보가 변경될 때 알림을 받아보시겠어요? '{사용자가 물어본 대상} 바뀌면 알려줘'라고 말씀하시면 자동으로 모니터링을 시작할 수 있어요."
